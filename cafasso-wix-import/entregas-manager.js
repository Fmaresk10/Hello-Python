(()=>{
  ...
  const isInternal=x=>String(x?.courseId||'').startsWith('__cafasso_');
  ...
  const getRows=()=>{
    ...
    return (state.submissions||[]).filter(x=>!isInternal(x)).filter(x=>(!q||[x.userName,x.userEmail,x.content,x.type,x.courseTitle,x.groupName].join(' ').toLowerCase().includes(q))&&(!st||(x.status||'Pendiente')===st)&&(!course||x.courseId===course)&&(!group||x.groupId===group)&&(!user||x.userId===user));
  };
  function install(){
  ...
  function fillUserFilter(){const sel=document.getElementById('userFilter');if(!sel)return;const cur=sel.value;const users=[...new Map((state.submissions||[]).filter(x=>!isInternal(x)&&x.userId).map(x=>[x.userId,{id:x.userId,name:x.userName||x.userEmail||'Animador'}])).values()].sort((a,b)=>a.name.localeCompare(b.name,'es'));sel.innerHTML='<option value="">Todos los animadores</option>'+users.map(u=>`<option value="${esc(u.id)}">${esc(u.name)}</option>`).join('');sel.value=cur;}
  const originalStats=stats;stats=function(){const old=state.submissions;state.submissions=(old||[]).filter(x=>!isInternal(x));try{return originalStats()}finally{state.submissions=old}};
  const originalFill=fillFilters;fillFilters=function(){originalFill();fillUserFilter()};
  render=function(){const all=(state.submissions||[]).filter(x=>!isInternal(x)),rows=getRows();$('resultNote').textContent=rows.length===all.length?`${all.length} entregas en total`:`Mostrando ${rows.length} de ${all.length} entregas`;$('body').innerHTML=rows.length?rows.map(x=>`<tr><td><strong>${esc(x.userName||'Animador')}</strong><br><small>${esc(x.groupName||x.userEmail||'')}</small></td><td>${esc(x.courseTitle||'Curso')}</td><td>${esc(x.type||'Entrega')}</td><td>${esc((x.content||'').slice(0,90))}${(x.content||'').length>90?'…':''}</td><td>${badge(x.status)}</td><td>${fmt(x._updatedDate||x._createdDate)}</td><td><button class="btn alt" data-open="${x._id}">Revisar</button></td></tr>`).join(''):'<tr><td colspan="7" class="empty">No hay entregas que coincidan con los filtros.</td></tr>';};
  function decorateRows(){document.querySelectorAll('#body [data-open]').forEach(btn=>{const row=btn.closest('tr'),item=(state.submissions||[]).find(x=>x._id===btn.dataset.open),reward=Number(item?.rewardAlmitas||0);if(row&&reward>0&&!row.querySelector('.almitas-cell')){const cell=row.querySelector('td:nth-child(4)');if(cell)cell.insertAdjacentHTML('beforeend',`<span class="almitas-cell">⭐ ${reward} almitas</span>`);}});}
  const originalRenderWithRewards=render;render=function(){originalRenderWithRewards();decorateRows();};
  ...
  document.addEventListener('click',e=>{if(e.target.closest('[data-open]'))setTimeout(()=>{if(!current)return;const reward=Number(current.rewardAlmitas||0);const meta=$('meta');if(meta&&reward>0&&!meta.querySelector('.almitas-modal'))meta.insertAdjacentHTML('beforeend',`<span class="almitas-modal">⭐ ${reward} almitas</span>`);},0);});
