(() => {
  ...
  function styles() {
    ...
    style.textContent = `
      ...
      .cafasso-world-return{position:fixed;z-index:100;left:max(18px,calc(env(safe-area-inset-left) + 14px));right:auto;top:max(18px,calc(env(safe-area-inset-top) + 14px));appearance:none;border:1px solid rgba(244,216,137,.55);background:rgba(8,35,39,.82);backdrop-filter:blur(10px);color:#FFF9E8;border-radius:999px;padding:10px 15px;font:850 11px Inter,system-ui;cursor:pointer;box-shadow:0 8px 22px rgba(0,0,0,.24)}.cafasso-world-return:hover{background:#F1C85B;color:#17302F}html[data-cafasso-preview-role] .cafasso-world-return,html[data-cafasso-exact-preview] .cafasso-world-return{top:max(58px,calc(env(safe-area-inset-top) + 54px))}@media(max-width:680px){.cafasso-world-return{left:max(10px,calc(env(safe-area-inset-left) + 7px));right:auto;top:max(10px,calc(env(safe-area-inset-top) + 7px));padding:9px 12px;font-size:10px}html[data-cafasso-preview-role] .cafasso-world-return,html[data-cafasso-exact-preview] .cafasso-world-return{top:max(58px,calc(env(safe-area-inset-top) + 54px))}}.cafasso-journey-mode .cafasso-course-map{background-image:url('https://static.wixstatic.com/media/47bf07_0d0a5e3ec41543cbb9d6171058171b28~mv2.png')!important}.cafasso-mission-mode{background-image:url('https://static.wixstatic.com/media/47bf07_0d0a5e3ec41543cbb9d6171058171b28~mv2.png')!important}
  ...
  function prepareWorldReturnButton() {
    ...
    if (!returnButton) {
      ...
      returnButton.textContent = page === 'course-player.html' ? '← Volver a Escuela' : '← Volver al mundo';
  ...
  function decorateStagePath() {
    ...
    map.querySelector('[data-next-module]')?.addEventListener('click', event => {
      ...
      const nextId = String(button.dataset.nextModule || nextModule?._id || nextModule?.id || '').trim();

      if (!nextId) return;

      if (typeof window.CafassoOpenModule === 'function' && window.CafassoOpenModule(nextId)) {
        return;
      }
