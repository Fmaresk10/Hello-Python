(()=>{
  ...
  function readSessionValue(key){try{return JSON.parse(sessionStorage.getItem(key)||'null')}catch(e){return null}}
  ...
  const exactPreviewId=String(initialParams.get('previewUser')||'').trim();
  let exactPreviewUser=null;
  if(isAdmin&&exactPreviewId){
    const stored=readSessionValue('cafassoExactPreviewUser');
    if(stored&&String(stored._id||'')===exactPreviewId&&String(stored.role||'').toLowerCase().includes('animador')){
      exactPreviewUser=stored;
      ...
      const virtualSession={...session,user:{...session.user,...exactPreviewUser,role:'Animador'}};
      Storage.prototype.getItem=function(key){
        if(this===localStorage&&String(key)==='cafassoSession')return JSON.stringify(virtualSession);
        return realGetItem.call(this,key);
      };
      window.CafassoExactPreview={active:true,user:exactPreviewUser};
    }
  }

  function installReadOnlyGuard(){
    if(!exactPreviewUser)return;
    const style=document.createElement('style');
    style.id='cafassoExactPreviewReadOnlyStyles';
    style.textContent=`
      html[data-cafasso-exact-preview] #logout,
      html[data-cafasso-exact-preview] #mobileLogout,
      html[data-cafasso-exact-preview] #savePassword,
      html[data-cafasso-exact-preview] #profilePassword,
      html[data-cafasso-exact-preview] #profilePassword2,
      html[data-cafasso-exact-preview] #cafassoChooseProfilePhoto,
      html[data-cafasso-exact-preview] #cafassoRemoveProfilePhoto,
      html[data-cafasso-exact-preview] [data-save-answer],
      html[data-cafasso-exact-preview] [data-review],
      html[data-cafasso-exact-preview] #completeModule{display:none!important}
    ...
    document.addEventListener('click',e=>{
      if(e.target.closest('#logout,#mobileLogout,#savePassword,#cafassoChooseProfilePhoto,#cafassoRemoveProfilePhoto,[data-save-answer],[data-review],#completeModule')){
        e.preventDefault();e.stopImmediatePropagation();
      }
    },true);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',installReadOnlyGuard,{once:true});else installReadOnlyGuard();
  ...
  function installRolePreview(){
    ...
    let preview=exactPreviewUser?'':String(params.get('previewRole')||'').toLowerCase();
    ...
    style.textContent=`
      ...
      .cafasso-role-options{display:grid;gap:10px}.cafasso-role-option{border:1px solid #E8DCCB;background:#fff;border-radius:15px;padding:14px;text-align:left;cursor:pointer;color:#0F2D4D}.cafasso-role-option:hover{border-color:#D9BE67;background:#FFFDF5}.cafasso-role-option strong{display:block;font-size:14px}.cafasso-role-option small{display:block;color:#687386;margin-top:3px}
      .cafasso-role-option.person{display:grid;grid-template-columns:42px 1fr;align-items:center;gap:11px}.cafasso-role-person-avatar{width:42px;height:42px;border-radius:50%;background:#F2C94C;display:grid;place-items:center;font-weight:900;color:#0F2D4D}.cafasso-role-person-copy small{margin-top:2px}
      .cafasso-role-divider{display:flex;align-items:center;gap:9px;color:#8B7650;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;margin:4px 0}.cafasso-role-divider:before,.cafasso-role-divider:after{content:'';height:1px;background:#E8DCCB;flex:1}
      .cafasso-role-search{width:100%;padding:12px 13px;border:1px solid #E8DCCB;border-radius:12px;background:#fff;font:inherit;margin-bottom:12px}.cafasso-role-list{display:grid;gap:8px;max-height:390px;overflow:auto;padding-right:2px}
      ...
      #cafasso-role-preview-bar{position:fixed;left:0;right:0;top:0;z-index:130;min-height:44px;background:#F2C94C;color:#0F2D4D;display:flex;align-items:center;justify-content:center;gap:12px;padding:8px 14px;font:800 12px Inter,system-ui;box-shadow:0 5px 16px rgba(15,45,77,.12)}
    ...
    function initials(name){const p=String(name||'').trim().split(/\s+/);return ((p[0]?.[0]||'A')+(p[1]?.[0]||'')).toUpperCase();}
    function roleModal(){
      overlay.innerHTML=`<div class="cafasso-role-preview-modal"><h3>Ver CAFASSO como…</h3><p>La vista previa no cambia tu cuenta ni tus permisos reales.</p><div class="cafasso-role-options">${allowed.map(r=>`<button class="cafasso-role-option" data-preview-role="${r}"><strong>${labels[r]}</strong><small>${descriptions[r]}</small></button>`).join('')}${isAdmin?`<div class="cafasso-role-divider">Usuario real</div><button class="cafasso-role-option" data-preview-user-picker><strong>👤 Elegir animador…</strong><small>Ver exactamente sus cursos, progreso y entregas en modo solo lectura.</small></button>`:''}</div><button class="cafasso-role-close" type="button">Cancelar</button></div>`;
    }
    function renderPickerList(filter=''){
      const list=overlay.querySelector('#cafasso-role-user-list');if(!list)return;
      const q=String(filter||'').trim().toLowerCase();
      const rows=pickerUsers.filter(u=>!q||`${u.name||''} ${u.email||''} ${u.groupName||''}`.toLowerCase().includes(q));
      list.innerHTML=rows.length?rows.map(u=>`<button class="cafasso-role-option person" data-preview-user-id="${u._id}"><span class="cafasso-role-person-avatar">${initials(u.name)}</span><span class="cafasso-role-person-copy"><strong>${String(u.name||'Animador').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}</strong><small>${String(u.groupName||'Sin grupo').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))} · ${String(u.email||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}</small></span></button>`).join(''):'<div style="padding:22px;text-align:center;color:#687386">No encontramos animadores con esa búsqueda.</div>';
    }
    async function showUserPicker(){
      if(!isAdmin)return;
      overlay.innerHTML='<div class="cafasso-role-preview-modal"><h3>Elegir animador</h3><p>Cargando usuarios de CAFASSO…</p></div>';
      try{
        const r=await fetch(ADMIN_API,{cache:'no-store'}),j=await r.json();
        if(!r.ok||!j.ok)throw new Error(j.error||'No se pudieron cargar los animadores.');
        pickerUsers=(j.users||[]).filter(u=>String(u.role||'').toLowerCase().includes('animador')&&!['Bloqueado','Inactivo'].includes(String(u.status||''))).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'es'));
        overlay.innerHTML='<div class="cafasso-role-preview-modal"><h3>Elegir animador</h3><p>Vas a ver CAFASSO con sus asignaciones y progreso reales. No vas a poder modificar nada.</p><input id="cafasso-role-user-search" class="cafasso-role-search" type="search" placeholder="Buscar por nombre, correo o grupo…" autocomplete="off"><div id="cafasso-role-user-list" class="cafasso-role-list"></div><button class="cafasso-role-close" data-preview-back type="button">← Volver</button></div>';
        renderPickerList('');
        setTimeout(()=>overlay.querySelector('#cafasso-role-user-search')?.focus(),0);
      }catch(error){
        overlay.innerHTML=`<div class="cafasso-role-preview-modal"><h3>No pudimos cargar los animadores</h3><p>${String(error.message||error)}</p><button class="cafasso-role-close" data-preview-back type="button">← Volver</button></div>`;
      }
    }
    function open(){roleModal();overlay.classList.add('show')}
    function close(){overlay.classList.remove('show')}
    function rememberReturn(){try{if(!sessionStorage.getItem('cafassoPreviewReturn'))sessionStorage.setItem('cafassoPreviewReturn',location.href.replace(/([?&])(previewRole|previewUser)=[^&]*&?/g,'$1').replace(/[?&]$/,''));}catch(e){}}
    function clearExact(){try{sessionStorage.removeItem('cafassoExactPreviewUser');}catch(e){}}
    function launchExact(user){
      if(!user)return;
      rememberReturn();
      try{sessionStorage.setItem('cafassoExactPreviewUser',JSON.stringify(user));}catch(e){}
      location.href='./?previewUser='+encodeURIComponent(user._id);
    }

    overlay.addEventListener('click',e=>{
      if(e.target===overlay||e.target.closest('.cafasso-role-close:not([data-preview-back])')){close();return;}
      if(e.target.closest('[data-preview-back]')){roleModal();return;}
      if(e.target.closest('[data-preview-user-picker]')){showUserPicker();return;}
      const ub=e.target.closest('[data-preview-user-id]');
      if(ub){launchExact(pickerUsers.find(u=>String(u._id)===String(ub.dataset.previewUserId)));return;}
      const b=e.target.closest('[data-preview-role]');
      if(!b)return;
      rememberReturn();clearExact();
      location.href=targets[b.dataset.previewRole];
    });
    overlay.addEventListener('input',e=>{if(e.target?.id==='cafasso-role-user-search')renderPickerList(e.target.value);});
    window.CafassoRolePreview={open,close,openUserPicker:showUserPicker};

    if(exactPreviewUser){
      const bar=document.createElement('div');
      bar.id='cafasso-role-preview-bar';
      bar.innerHTML=`<span>👁 Viendo como <strong>${String(exactPreviewUser.name||'Animador')}</strong> · Solo lectura</span><span><button type="button" data-change-user>Cambiar animador</button> <button type="button" data-exit-preview>Salir</button></span>`;
      document.body.appendChild(bar);
      bar.querySelector('[data-change-user]').onclick=()=>{overlay.classList.add('show');showUserPicker();};
  ...
  window.fetch=async function(input,init={}){
    ...
    if(exactPreviewUser&&!['GET','HEAD','OPTIONS'].includes(method)){
      return new Response(JSON.stringify({ok:false,error:'Estás en una vista previa de solo lectura. Salí de la vista previa para modificar datos.'}),{status:403,headers:{'Content-Type':'application/json'}});
    }
...
// CAFASSO deploy marker: exact-user-preview-v1
