(()=>{
  ...
  style.textContent=`
    .quick-content-bar{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:13px 0 5px}.quick-content-bar button{border:1px solid #d9ccb4;background:#fff9ed;color:#3e584d;border-radius:7px;padding:9px 10px;font-size:11px;font-weight:850;cursor:pointer;text-align:left}.quick-content-bar button:hover{background:#f1e4c7;border-color:#c5aa69}
    .module-jump{display:flex;gap:7px;align-items:center;margin:0 0 13px;padding:7px;background:#eee4d2;border:1px solid #ddcfb6;border-radius:8px}.module-jump select{flex:1;border:1px solid #d6c7ad;background:#fff9ed;border-radius:6px;padding:9px 10px;color:#385047;font:inherit}.module-jump button{border:1px solid #d6c7ad;background:#fff9ed;color:#385047;border-radius:6px;padding:9px 10px;font-weight:850;cursor:pointer}
    .block-quick-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:13px;padding-top:13px;border-top:1px solid #dfd2bd}.block-quick-actions button{border:1px solid #d6c8af;background:#fff9ed;color:#40584e;border-radius:6px;padding:8px 10px;font-size:11px;font-weight:850;cursor:pointer}.block-quick-actions .danger{color:#995347;border-color:#e0bdb6;background:#fff5f2}
    .editor-shortcuts{font-size:11px;color:#68655f;margin-top:8px}.editor-shortcuts kbd{background:#f7edda;border:1px solid #d7c9af;border-bottom-width:2px;border-radius:5px;padding:2px 5px;font:800 10px Inter,system-ui;color:#40564c}
    .editor-mobile-actions{display:none}
    @media(max-width:700px){
      body{padding-bottom:80px}.side{overflow-x:auto}.side-tools{min-width:max-content}.quick-content-bar{display:grid;grid-template-columns:repeat(2,1fr)}.quick-content-bar button{border-radius:7px;min-height:42px}.module-jump{position:sticky;top:0;z-index:8;background:var(--cream);padding:8px 0}.editor-mobile-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;position:fixed;left:0;right:0;bottom:0;z-index:95;background:#fffaf0;backdrop-filter:none;border-top:1px solid var(--line);padding:9px 12px calc(9px + env(safe-area-inset-bottom));box-shadow:0 -8px 24px rgba(15,45,77,.10)}.editor-mobile-actions button{min-height:46px;border:0;border-radius:12px;font-weight:850}.editor-mobile-actions .save{background:#fff;border:1px solid var(--line);color:var(--navy)}.editor-mobile-actions .publish{background:var(--green);color:#fff}
    }
  `;
  ...
  if(!document.querySelector('script[data-cafasso-templates]')){
    const s=document.createElement('script');s.src='./course-templates.js?v=20260911-1';s.defer=true;s.dataset.cafassoTemplates='1';document.body.appendChild(s);
  }
  ...
  const loadEvaluationEditor=()=>{
    if(document.querySelector('script[data-cafasso-evaluation-editor]'))return;
    const s=document.createElement('script');s.src='./course-evaluation-editor.js?v=20260921-2';s.defer=true;s.dataset.cafassoEvaluationEditor='1';document.body.appendChild(s);
  };
  if(!document.querySelector('script[data-cafasso-typed-blocks]')){
    const s=document.createElement('script');s.src='./course-block-editor.js?v=20260921-3';s.defer=true;s.dataset.cafassoTypedBlocks='1';s.onload=loadEvaluationEditor;document.body.appendChild(s);
  }else{
    loadEvaluationEditor();
  }
