(() => {
  ...
  const DESTINATIONS = {
    house: {
      ...
      aria: 'Volver a la Casa',
      image: './assets/cafasso-casa-interior-v2.jpg'
    },
    escuela: {
      main: 'Escuela',
      sub: 'seguir aprendiendo',
      aria: 'Entrar a la Escuela',
      image: 'https://static.wixstatic.com/media/47bf07_481618e0256044f9b31ae360a03a9169~mv2.png'
    },
    parroquia: {
      main: 'Parroquia',
      sub: 'hacer silencio',
      aria: 'Entrar a la Parroquia',
      image: 'https://static.wixstatic.com/media/47bf07_b411bebc79644a0092920abb58a2f56d~mv2.png'
    }
  };
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .cafasso-patio-transition.is-active{opacity:1}
      .cafasso-patio-transition.is-active:after{opacity:.84}

      @media(max-width:680px){
        .cafasso-patio .cafasso-space-link{min-width:94px;padding:8px 10px 7px}
        .cafasso-patio-link__main{font-size:12px}.cafasso-patio-link__sub{font-size:5.5px}
        .cafasso-patio-intro{bottom:3.8%;width:88vw;padding-bottom:10px}.cafasso-patio-intro strong{font-size:24px}.cafasso-patio-intro span{font-size:7px}
      }
  ...
  function createMurmur() {
    try {
      ...
      for (let i = 0; i < length; i += 1) {
        ...
        const voices = .54 + Math.sin(t * 2.1) * .11 + Math.sin(t * 3.7 + 1.2) * .07;
        data[i] = (low * .72 + mid * .28) * voices;
      }
      ...
      source.start();
      gain.gain.exponentialRampToValueAtTime(.018, ctx.currentTime + 1.1);
  ...
  function stopAmbience() {
    ...
    try {
      ...
      ambience.gain.gain.setValueAtTime(Math.max(.0001, ambience.gain.gain.value || .018), now);
      ambience.gain.gain.exponentialRampToValueAtTime(.0001, now + .24);
      setTimeout(() => {
        try { ambience.source.stop(); } catch (error) {}
  ...
  function boot() {
    ...
    Object.entries(DESTINATIONS).forEach(([space, meta]) => {
      const selector = space === 'house'
        ? '.cafasso-space-link--patio-home[data-space="house"]'
        : `.cafasso-space-link[data-space="${space}"]`;
    ...
    const unlock = () => startAmbience();
    patio.addEventListener('pointerdown', unlock, { once: true, passive: true });
    patio.addEventListener('keydown', unlock, { once: true });
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      else if (ambienceStarted && !ambience) {
        ambienceStarted = false;
      }
    });
    window.addEventListener('pagehide', stopAmbience, { once: true });
    return true;
  }
